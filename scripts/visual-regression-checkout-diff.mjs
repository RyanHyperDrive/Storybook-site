#!/usr/bin/env node
/**
 * Compare freshly-captured /checkout screenshots against committed baselines
 * using pixelmatch. Fails the build when the per-image diff exceeds a small
 * threshold.
 *
 * Layout:
 *   visual-regression/checkout/baseline/<name>.png   ← committed
 *   visual-regression/checkout/<name>.png            ← from capture script
 *   visual-regression/checkout/diff/<name>.png       ← diff artifact
 *
 * Env knobs:
...
const ROOT = resolve("visual-regression", "checkout");
...
if (screenshots.length === 0) {
  console.error(
    "No screenshots in visual-regression/checkout/. " +
      "Run scripts/visual-regression-checkout.mjs first.",
  );
  process.exit(1);
}
