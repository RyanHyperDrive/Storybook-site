#!/usr/bin/env node
/**
 * Compare freshly-captured /create-journey screenshots against committed
 * baselines using pixelmatch. Fails the build when the per-image diff
 * exceeds a small threshold.
 *
 * Layout:
 *   visual-regression/create-journey/baseline/<name>.png   ← committed
 *   visual-regression/create-journey/<name>.png            ← produced by the
 *                                                            journey script
 *   visual-regression/create-journey/diff/<name>.png       ← diff artifact
 *
 * Env knobs:
 ...
 *   UPDATE_BASELINES=1     overwrite baselines from the latest screenshots
 *                          and skip diffing. Use locally to refresh.
 *
 * Usage:
...
const ROOT = resolve("visual-regression", "create-journey");
...
if (screenshots.length === 0) {
  console.error(
    "No screenshots in visual-regression/create-journey/. " +
      "Run scripts/visual-regression-create-journey.mjs first.",
  );
  process.exit(1);
}
