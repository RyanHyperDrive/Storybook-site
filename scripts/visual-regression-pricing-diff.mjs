#!/usr/bin/env node
/**
 * Compare freshly-captured /pricing screenshots against committed baselines
 * using pixelmatch. Fails the build when the per-image diff exceeds a small
 * threshold.
 *
 * Layout:
 *   visual-regression/pricing/baseline/<name>.png   ← committed
 *   visual-regression/pricing/<name>.png            ← from capture script
 *   visual-regression/pricing/diff/<name>.png       ← diff artifact
 *
 * Env knobs:
...
const ROOT = resolve("visual-regression", "pricing");
...
if (screenshots.length === 0) {
  console.error(
    "No screenshots in visual-regression/pricing/. " +
      "Run scripts/visual-regression-pricing.mjs first.",
  );
  process.exit(1);
}
