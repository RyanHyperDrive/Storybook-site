...
- wrong_character_details = list of IDENTITY attributes that are wrong (e.g. "hair is brown instead of red", "skin lightened from deep brown to tan", "skin darkened / blackened from the character sheet", "coily hair rendered as loose waves"). The skin-tone check is SYMMETRIC: ANY deviation from the character sheet's skin tone MUST be flagged — LIGHTENED, darkened, blackened, desaturated, shadowed-out, or shifted in undertone — and must drop character_consistency_score to 0.4 or lower. Straightening or loosening textured/coily hair, narrowing a wide nose, thinning full lips, or otherwise softening ethnic features compared to the character sheet must also be flagged here and drop character_consistency_score to 0.4 or lower. SILHOUETTE / DARK-RENDERING HARD FAILURE: if ANY main character is rendered as a silhouette, blacked-out shape, or is so dark/dim/shadowed that their face and true skin tone are not clearly visible, treat it as a hard failure — add "character is silhouetted / skin tone darkened — must be clearly lit in true skin tone" to wrong_character_details, drop character_consistency_score to 0.3 or lower, and set needs_regeneration = true. Do NOT flag scene-appropriate clothing (e.g. pajamas in a bedtime scene, a costume during a costume scene) as a wrong character detail, and do NOT use outfit differences from the character sheet to lower character_consistency_score. If the clothing genuinely conflicts with what the page's required scene calls for, note it in "composition_issues" as an outfit/scene mismatch instead.
